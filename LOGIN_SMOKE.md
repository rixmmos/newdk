...
| File | What changed |
|---|---|
| `dkrixserver/conf/loginserver.conf` | `HomePath`, Billing/DB/Log hosts: `192.168.0.16` + `/home/genius/...` → `127.0.0.1` + `/mnt/c/newdk/dkrixserver` |
...
```bash
mysql -uelcastle -pelca110 DARKEDEN < /mnt/c/newdk/create_test_account.sql
```
...
```bash
bash /mnt/c/newdk/start_servers.sh start
```
...
```bash
tail -80 /mnt/c/newdk/dkrixserver/sharedserver.log
tail -80 /mnt/c/newdk/dkrixserver/loginserver.log
tail -80 /mnt/c/newdk/dkrixserver/gameserver.log
```
...
- **`HomePath does not exist`** — a server logs this and aborts if it
  ...
  all three confs point at `/mnt/c/newdk/dkrixserver`.
- **Port already in use** — a prior crashed server still listening.
  `bash /mnt/c/newdk/start_servers.sh stop` kills them.

To watch all three logs live while clicking through the UI:

```bash
bash /mnt/c/newdk/start_servers.sh tail
```
...
```bash
bash /mnt/c/newdk/client_smoke.sh
```
...
bash /mnt/c/newdk/start_servers.sh stop
```
