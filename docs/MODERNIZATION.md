...
### Phase 14 — Explicit CMake source lists — plan 2026-04-19
...
- [ ] Replace the larger `file(GLOB CLIENT_MAIN_SOURCES
      ...
      ~1045 files. Largest source list in the tree. **Scope
      audit (2026-04-19):** 242 files under `Client/` root +
      801 under `Client/Packet/` (recursive) + 1 under
      `Client/SXml/` + 1 under `Client/WinLib/` ⇒ 1045 files.
      Note: line 482's existing glob includes both
      `Client/Packet/*.cpp` AND `Client/Packet/**/*.cpp` — the
      second subsumes the first, so the explicit list dedupes
      to the single recursive walk.
...
- **14A — Pin plan.** (This commit.) Adds the sub-audit numbers
  for the two glob expansions (56 VS_UI files, 1045 Client
  files) and sets the order of work below.
- **14B — VS_UI glob → explicit list.** Replace the 56-file
  `GLOB_RECURSE` at `dkrix/CMakeLists.txt:163`. Smaller, easier
  to review first.
- **14C — Client glob → explicit list.** Replace the 1045-file
  `GLOB` at `dkrix/CMakeLists.txt:482`. The large commit;
  generated reproducibly by `find Client -name "*.cpp" | sort`
  so the list can be regenerated byte-for-byte on any Unix
  checkout of the tree.
- **14D — Close-out + CLAUDE.md workflow note.** Flip to
  `done`, outcome block, commit table. Append an "Adding new
  source files" section to `dkrix/CLAUDE.md` so future
  contributors know they have to edit the list (CMake won't
  pick up newly-dropped `.cpp` files automatically anymore).
