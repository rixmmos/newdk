...
| 2026-04-21 | `CGStoreSign`  |            320 | Completes the store-UI trio's shared-header adoption. Server-side `PacketFactoryManager.cpp` include-only path note is broadened to cover `CGStore{Open,Close,Sign}.h`; `GameServerPackets` now absorbs `CGStoreSign.cpp` from `shared_packets`, while the client stub swaps from `Packet/Cpackets/CGStoreSign.h` to the shared header and drops the old throw-spec to match it. Remaining first-wave cosmetic-`.cpp` candidates: `CGDisplayItem`, `CGGQuestCancel`, `CGRequestStoreInfo`, `CGUndisplayItem` (4). |
| 2026-04-21 | `CGRequestStoreInfo` |            318 | Keeps the store-UI family together. `GameServerPackets` now absorbs `CGRequestStoreInfo.cpp` from `shared_packets`; the include-only note for `PacketFactoryManager.cpp` is broadened to cover `CGRequestStoreInfo.h` alongside `CGStore{Open,Close,Sign}.h`; client-side `CGHandlersStub.cpp`, `UIMessageManager.cpp`, and `PacketFactoryManager.cpp` now resolve the shared header and drop the legacy throw-spec on the stub to match it. Remaining first-wave cosmetic-`.cpp` candidates: `CGDisplayItem`, `CGGQuestCancel`, `CGUndisplayItem` (3). |
| 2026-04-21 | `CGDisplayItem` |            316 | Starts closing out the non-store canaries. `GameServerPackets` now absorbs `CGDisplayItem.cpp` from `shared_packets`; the include-only `PacketFactoryManager.cpp` path note is broadened again for `CGDisplayItem.h`; client-side `CGHandlersStub.cpp`, `UIMessageManager.cpp`, and `PacketFactoryManager.cpp` now include the shared header, and the stub drops its legacy throw-spec to match it. Remaining first-wave cosmetic-`.cpp` candidates: `CGGQuestCancel`, `CGUndisplayItem` (2). |
| 2026-04-21 | `CGGQuestCancel` |            314 | This one keeps the client-side `__GAME_CLIENT__` gating shape instead of using a stub: shared `execute()` only dispatches to `CGGQuestCancelHandler` outside the game client, matching the old split build behavior. `GameServerPackets` now absorbs `CGGQuestCancel.cpp` from `shared_packets`; client-side `UIMessageManager.cpp` and `PacketFactoryManager.cpp` now include the shared header. Remaining first-wave cosmetic-`.cpp` candidates: `CGUndisplayItem` (1). |
| 2026-04-21 | `CGUndisplayItem` |            312 | Closes out the original Phase 17 cosmetic-`.cpp` canary shortlist. `GameServerPackets` now absorbs `CGUndisplayItem.cpp` from `shared_packets`; client-side `CGHandlersStub.cpp`, `UIMessageManager.cpp`, and `PacketFactoryManager.cpp` now include the shared header and the stub drops its legacy throw-spec to match it. Remaining first-wave cosmetic-`.cpp` candidates: none (0). |
| 2026-04-21 | `CGBuyStoreItem` |            310 | First post-canary semantic migration, still within the store-UI cluster. `GameServerPackets` now absorbs `CGBuyStoreItem.cpp` from `shared_packets`; client-side `CGHandlersStub.cpp`, `UIMessageManager.cpp`, and `PacketFactoryManager.cpp` now include the shared header, and the shared factory keeps `throw()` specs so it satisfies the client's `PacketFactory` base declarations. |
| 2026-04-21 | `GCMyStoreInfo` + `GCOtherStoreInfo` + `GCRemoveStoreItem` + `GCAddStoreItem` |            310 | Store-UI receive-side cluster migrated as one batch. The shared `GCMyStoreInfo` and `GCOtherStoreInfo` retain client-side null-init plus allocate-before-read behavior for `StoreInfo*`, avoiding the server copy's unsafe assumption that `m_pInfo` is already populated before decoding. Client-side `UIMessageManager.cpp` and `PacketFactoryManager.cpp` now include the shared headers; server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets` while the handler `.cpp` files remain server-local. The Phase 12 duplicate ratchet is unchanged here because it currently measures only `Client/Packet/Cpackets/` intersections, not `Gpackets/`. |
| 2026-04-21 | `CGShopRequestList` + `CGShopRequestBuy` + `CGShopRequestSell` |            304 | Shop-request send-side cluster migrated as one batch. The shared packet classes keep the old client-safe `__GAME_CLIENT__` execute-gating so the client does not need local handler stubs, while their factories preserve legacy `throw()` specs to stay compatible with the client's `PacketFactory` declarations. Client-side `PacketFactoryManager.cpp`, `RequestClientPacketFactoryManager.cpp`, and `RequestServerPacketFactoryManager.cpp` now include the shared headers; server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`. |
| 2026-04-21 | `CGStashList` + `CGStashDeposit` + `CGStashWithdraw` |            298 | Stash request cluster migrated as one batch. The shared packet classes keep the client-safe `__GAME_CLIENT__` execute-gating and preserve legacy `throw()` factory signatures for client compatibility. Client-side `PacketFactoryManager.cpp`, `RequestClientPacketFactoryManager.cpp`, and `RequestServerPacketFactoryManager.cpp` now include the shared headers; server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`. |
| 2026-04-21 | `CGMouseToStash` + `CGStashToMouse` + `CGStashRequestBuy` |            292 | Stash item-transfer cluster migrated as one batch. The shared packet classes keep the client-safe `__GAME_CLIENT__` execute-gating and preserve legacy `throw()`-style factory compatibility, while `PacketDef.h` now points these stash request headers at `shared/Packets` so the wider client tree resolves them without local `Cpackets` copies. Client-side `PacketFactoryManager.cpp`, `RequestClientPacketFactoryManager.cpp`, and `RequestServerPacketFactoryManager.cpp` now include the shared headers; server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`. |
| 2026-04-21 | `CGTradePrepare` + `CGTradeAddItem` + `CGTradeRemoveItem` + `CGTradeMoney` + `CGTradeFinish` |            282 | Trade request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating, preserve legacy `throw()`-compatible factory and handler signatures for the mixed client/server tree, and repoint `PacketDef.h` plus all three client factory-manager files at `shared/Packets` so the wider trade UI path no longer depends on local `Cpackets` copies. Server-side `GameServerPackets` now absorbs all five `.cpp` files from `shared_packets`. |
| 2026-04-21 | `CGAddSMSAddress` + `CGDeleteSMSAddress` + `CGSMSAddressList` |            276 | SMS address-book request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory signatures, while the server-side constructors/destructors were aligned to the client-safe non-rethrow pattern used by adjacent migrated request packets. `PacketFactoryManager.cpp` now includes the shared headers; server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`. `CGSMSSend` was intentionally left out of this batch because it still carries a real protocol/behavior divergence (`MAX_MESSAGE_LENGTH` and receiver-list decode behavior) rather than the standard templated migration shape. |
| 2026-04-21 | `CGAcceptUnion` + `CGDenyUnion` + `CGQuitUnionAccept` + `CGQuitUnionDeny` |            268 | Union-decision request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory/handler signatures, while client-side `PacketFactoryManager.cpp` and `UIMessageManager.cpp` now include the shared headers instead of local `Cpackets` copies. Server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGRequestUnion` + `CGRequestUnionInfo` + `CGQuitUnion` + `CGExpelGuild` |            260 | Union/guild request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory/handler signatures, while client-side `PacketFactoryManager.cpp` and `UIMessageManager.cpp` now include the shared headers instead of local `Cpackets` copies. Server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGRequestGuildList` + `CGAppointSubmaster` + `CGModifyTaxRatio` |            254 | Guild-admin request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory/handler signatures, while client-side `PacketFactoryManager.cpp` and `UIMessageManager.cpp` now include the shared headers instead of local `Cpackets` copies. Server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGRangerSay` |            252 | Ranger-chat request migrated as an explicit semantic pass rather than a templated batch. The shared packet now follows the server-side `CGRangerSayHandler` dispatch with the usual client-safe `__GAME_CLIENT__` execute-gating; the old client copy's `CGWhisperHandler` dispatch was dead code behind the client guard and treated as rename/copy-paste drift, not canonical behavior. Client-side `UIMessageManager.cpp` now includes the shared header, and server-side `GameServerPackets` absorbs `CGRangerSay.cpp` from `shared_packets`. |
| 2026-04-21 | `CGRequestGuildMemberList` + `CGModifyGuildIntro` + `CGModifyGuildMemberIntro` + `CGModifyGuildMember` |            244 | Guild-member admin cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory/handler signatures, while client-side `PacketFactoryManager.cpp`, `UIMessageManager.cpp`, and `SizeOfObjects.cpp` now include the shared headers instead of local `Cpackets` copies. Server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGSelectGuild` + `CGSelectGuildMember` + `CGTryJoinGuild` + `CGJoinGuild` |            236 | Guild-selection/join cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory/handler signatures, while client-side `PacketFactoryManager.cpp`, `UIMessageManager.cpp`, and `SizeOfObjects.cpp` now include the shared headers instead of local `Cpackets` copies. `CGJoinGuild` preserves the client-side intro-length validation in the shared canonical version so the migration does not relax an existing wire check. Server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGRegistGuild` + `CGGuildChat` + `CGWithdrawTax` |            230 | Guild economy/chat cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and legacy `throw()`-compatible factory/handler signatures, while client-side `PacketFactoryManager.cpp`, `UIMessageManager.cpp`, and `SizeOfObjects.cpp` now include the shared headers instead of local `Cpackets` copies. `CGRegistGuild` preserves the client-side guild-intro length validation in the shared canonical version, and `CGGuildChat` keeps both the legacy `SetType/GetType` naming and the lower-case `setType/getType` accessors so existing client and server call sites continue to compile unchanged. Server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGAddGearToMouse` + `CGAddInventoryToMouse` + `CGAddMouseToGear` + `CGAddMouseToInventory` + `CGAddMouseToQuickSlot` + `CGAddMouseToZone` + `CGAddQuickSlotToMouse` + `CGAddZoneToInventory` + `CGAddZoneToMouse` |            212 | Inventory/mouse transfer cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating while `PacketDef.h`, `PacketFactoryManager.cpp`, `RequestClientPacketFactoryManager.cpp`, and `RequestServerPacketFactoryManager.cpp` now include the shared headers instead of local `Cpackets` copies. `CGAddInventoryToMouse` and `CGAddMouseToInventory` preserve the client-only `m_InventoryItemObjectID` side-state and its accessors in the shared canonical version without changing the serialized wire format. Server-side `GameServerPackets` absorbs all nine `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGRequestNewbieItem` + `CGRequestPowerPoint` + `CGRequestIP` + `CGRequestInfo` |            204 | Small request-packet cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, `RequestClientPacketFactoryManager.cpp`, `RequestServerPacketFactoryManager.cpp`, `UIMessageManager.cpp`, `MPlayer.cpp`, `ProfileManager.cpp`, `WhisperManager.cpp`, and `GCPartyJoinedHandler.cpp` were rewired away from deleted local `Cpackets` headers. Server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGSetSlayerHotKey` + `CGSetVampireHotKey` + `CGSelectRankBonus` |            198 | Hotkey/rank-selection cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while `PacketDef.h`, `PacketFactoryManager.cpp`, both request-side factory managers, and `UIMessageManager.cpp` now resolve the shared headers instead of deleted local `Cpackets` copies. Finishing this batch also cleaned up two earlier cross-build hazards in already-migrated shared packets: client-incompatible `__END_CATCH_NO_RETHROW` use in shared destructors was normalized, and the encrypted zone-transfer packets now fall back cleanly to the non-encrypted path when the `SocketEncrypt*Stream` downcast is unavailable instead of depending on a client-only assert macro surface. |
| 2026-04-21 | `CGAuthKey` + `CGCrashReport` + `CGDialUp` |            192 | Small auth/telemetry request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, `PacketFunction.cpp`, and `PacketDef.h` were rewired away from deleted local `Cpackets` headers. `CGCrashReport` also drops the last client-only `Assert.h` dependency from this batch by using explicit size validation before serializing instead of relying on divergent assert macro surfaces. Server-side `GameServerPackets` absorbs all three `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGPhoneDisconnect` + `CGPhoneSay` |            188 | Phone chat/disconnect mini-cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while `PacketDef.h` now resolves both headers from `shared/Packets` instead of the deleted local `Cpackets` copies. Server-side `GameServerPackets` absorbs both `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGGlobalChat` + `CGPartyPosition` + `CGPartySay` |            182 | Chat/party mini-cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `MPlayer.cpp`, and `UIMessageManager.cpp` were rewired away from deleted local `Cpackets` headers. `CGPartyPosition` also restores the client-used two-argument `setHP(maxHP, hp)` helper in the shared canonical API so existing call sites continue to compile unchanged. |
| 2026-04-21 | `CGPartyInvite` + `CGPartyLeave` + `CGFailQuest` + `CGGQuestAccept` |            174 | Party-control plus gquest mini-clusters migrated in one pass. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `GCPartyInviteHandler.cpp`, `MPlayer.cpp`, `SizeOfObjects.cpp`, and `UIMessageManager.cpp` were rewired away from deleted local `Cpackets` headers. Server-side `GameServerPackets` absorbs all four `.cpp` files from `shared_packets`; the server-local handler `.cpp` files remain under `src/Core/`. |
| 2026-04-21 | `CGLotterySelect` + `CGPetGamble` |            170 | Lottery/pet mini-cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, `CGHandlersStub.cpp`, and `UIMessageManager.cpp` were rewired away from deleted local `Cpackets` headers. Finishing this batch also aligned the client stub for `CGLotterySelectHandler::execute` with the shared header's non-throwing declaration so the broader client build stays consistent. |
| 2026-04-21 | `CGLogout` + `CGDownSkill` + `CGDonationMoney` + `CGPickupMoney` |            162 | Logout/skill/economy mini-clusters migrated in one pass. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `SizeOfObjects.cpp`, `UIMessageManager.cpp`, and `CGHandlersStub.cpp` were rewired away from deleted local `Cpackets` headers. `CGPickupMoney` also received the same shared-tree encrypted-stream fallback as earlier zone/item packets so the client build no longer depends on a client-only assert macro surface, and `CGDonationMoneyHandler`'s client stub was aligned with the shared non-throwing declaration. |
| 2026-04-21 | `CGConnect` + `CGNPCAskAnswer` |            158 | Connect/NPC-dialog mini-cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `GCNPCResponseHandler.cpp`, and `LCReconnectHandler.cpp` were rewired away from deleted local `Cpackets` headers. `CGConnect` preserves the client-only `setMacAddress(...)` helper in the shared canonical API, and `CGNPCAskAnswer` received the same encrypted-stream fallback used in the other migrated encrypted packets so the client build no longer depends on a client-only assert macro surface. |
| 2026-04-21 | `CGDissectionCorpse` + `CGGetOffMotorCycle` + `CGReady` + `CGPortCheck` + `CGAddItemToItem` + `CGAddItemToCodeSheet` + `CGModifyNickname` |            144 | Larger mixed request cluster migrated in one pass to reduce build churn. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `SizeOfObjects.cpp`, `UIMessageManager.cpp`, `GCNPCInfoHandler.cpp`, `GCUpdateInfoHandler.cpp`, and `LCReconnectHandler.cpp` were rewired away from deleted local `Cpackets` headers. `CGDissectionCorpse` now uses the shared encrypted-stream fallback pattern instead of the old assert-only downcast assumption, `CGModifyNickname` preserves the client-used nickname-ID accessor aliases in the shared API, and `CGPortCheck` was aligned to `DatagramPacket`'s stricter override exception surface so the client build stays valid. |
| 2026-04-21 | `CGSelectBloodBible` + `CGSelectNickname` + `CGSelectPortal` + `CGSelectQuest` + `CGSelectRegenZone` + `CGSelectTileEffect` + `CGSelectWayPoint` |            130 | Selection/request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory/handler `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `CGHandlersStub.cpp`, `MPlayer.cpp`, `UIDialog.cpp`, `SizeOfObjects.cpp`, and `UIMessageManager.cpp` were rewired away from deleted local `Cpackets` headers. `CGSelectPortal` also dropped a client-incompatible `__END_CATCH_NO_RETHROW` destructor tail so the shared tree now builds cleanly on both sides. |
| 2026-04-21 | `CGDepositPet` + `CGWithdrawPet` + `CGResurrect` + `CGTameMonster` |            122 | Creature/action request cluster migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory/handler `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `CGHandlersStub.cpp`, `SizeOfObjects.cpp`, and `UIMessageManager.cpp` were rewired away from deleted local `Cpackets` headers. `CGTameMonster` also drops the shared-tree `Assert1.h` dependency and the client-incompatible non-rethrow destructor tail, leaving the packet canonicalized in the same cross-build-safe shape as the other migrated request packets. |
| 2026-04-21 | `CLChangeServer` + `CLGetPCList` + `CLGetServerList` + `CLGetWorldList` + `CLLogout` + `CLSelectServer` + `CLSelectWorld` + `CLVersionCheck` |             90 | First half of the login/world/server `CL*` family migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory/handler `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `CWaitUIUpdate.cpp`, `GameUI.cpp`, `SizeOfObjects.cpp`, `UIMessageManager.cpp`, and the relevant `Lpackets` handlers were rewired away from deleted local `Cpackets` headers. This also moves the client/server login bootstrap packets onto the same shared include surface as the already-migrated `CG*` families, leaving the remaining `CLCreatePC`/`CLDeletePC`/`CLLogin`/`CLRegisterPlayer` side for the next pass. |
| 2026-04-21 | `CLCreatePC` + `CLDeletePC` + `CLLogin` + `CLQueryCharacterName` + `CLQueryPlayerID` + `CLReconnectLogin` + `CLRegisterPlayer` + `CLSelectPC` |             90 | Second half of the login/account `CL*` family migrated as one batch. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory/handler `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `SizeOfObjects.cpp`, `UIMessageManager.cpp`, `GCReconnectLoginHandler.cpp`, `LCPCListHandler.cpp`, and `LCQueryResultPlayerIDHandler.cpp` were rewired away from deleted local `Cpackets` headers. `CLLogin` and `CLReconnectLogin` also preserve the client-side login-mode helpers (`setMacAddress(...)`, `setLoginMode(...)`, `SetLoginMode(bool)`) in the shared canonical API so existing call sites continue to compile unchanged. |
| 2026-04-21 | `CGReloadFromInventory` + `CGReloadFromQuickSlot` + `CGUseBonusPoint` + `CGUseItemFromGear` + `CGUseItemFromInventory` + `CGUsePotionFromInventory` + `CGUsePotionFromQuickSlot` + `CGUsePowerPoint` |             74 | Reload/use-item cluster migrated as one batch to cut build churn. The shared packet classes keep client-safe `__GAME_CLIENT__` execute-gating and client-compatible factory/handler `throw()` specs, while client-side `PacketFactoryManager.cpp`, both request-side factory managers, `PacketDef.h`, `MItem.cpp`, `MPlayer.cpp`, `SizeOfObjects.cpp`, `UIMessageManager.cpp`, and `GCUseBonusPointOKHandler.cpp` were rewired away from deleted local `Cpackets` headers. `CGUseItemFromInventory` preserves the client-side `m_InventoryItemObjectID` helper state but keeps the three-field wire format that both old implementations actually serialized, and `CGUseMessageItemFromInventory.h` now inherits from the shared base header instead of the deleted local copy. |
