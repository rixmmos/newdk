...
```bash
ls -la /mnt/c/newdk/dkrixserver/bin/
file /mnt/c/newdk/dkrixserver/bin/gameserver
```
...
```bash
cd /mnt/c/newdk/dkrixserver/conf
...
dkrixserver tree lives at `/mnt/c/newdk/dkrixserver` so that's what
goes in.

```bash
sed -i 's|^HomePath : .*|HomePath : /mnt/c/newdk/dkrixserver|' \
...
1. `ls -la /mnt/c/newdk/dkrixserver/bin/` after build
