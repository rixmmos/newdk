...
struct PreparedResultColumn {
    enum Kind { KIND_INT64, KIND_UINT64, KIND_DOUBLE, KIND_STRING, KIND_TIME };
    ...
    PreparedResultColumn()
        : kind(KIND_STRING), mysqlType(MYSQL_TYPE_STRING), signedValue(0), unsignedValue(0), doubleValue(0.0),
          length(0), isNull(0), error(0) {
...
static std::string formatPreparedTime(const MYSQL_TIME& t, enum_field_types type) {
    ...
    if (type == MYSQL_TYPE_TIME) {
        if (t.day > 0) {
            std::snprintf(buffer, sizeof(buffer), "%s%u %02u:%02u:%02u", t.neg ? "-" : "", t.day, t.hour, t.minute,
                          t.second);
        } else {
            std::snprintf(buffer, sizeof(buffer), "%s%02u:%02u:%02u", t.neg ? "-" : "", t.hour, t.minute, t.second);
        }
        return buffer;
    }

    std::snprintf(buffer, sizeof(buffer), "%04u-%02u-%02u %02u:%02u:%02u", t.year, t.month, t.day, t.hour, t.minute,
                  t.second);
    return buffer;
}
...
PreparedStatement::PreparedStatement(Connection* pConnection, const std::string& sqlWithQMarks)
    : m_pConnection(pConnection), m_pStmt(NULL), m_Statement(sqlWithQMarks), m_nParamCount(0), m_nAffectedRows(0),
      m_nInsertID(0), m_pResult(NULL) {
    __BEGIN_TRY
...
void PreparedStatement::checkIdx(uint idx) const {
    ...
    if (idx == 0 || idx > m_nParamCount) {
        char msg[256];
        std::snprintf(msg, sizeof(msg), "PreparedStatement::bind*: idx %u out of range [1..%u] for stmt [%s]", idx,
                      m_nParamCount, m_Statement.c_str());
...
Result* PreparedStatement::execute() {
    ...
    for (uint i = 0; i < m_nParamCount; ++i) {
        if (m_Params[i].type == Param::PARAM_UNSET) {
            char msg[256];
            std::snprintf(msg, sizeof(msg), "PreparedStatement::execute: parameter %u unbound for stmt [%s]", i + 1,
                          m_Statement.c_str());
