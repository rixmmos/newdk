...
void CBillingPlayer::sendLogin(LoginPlayer* pLoginPlayer) {
    ...
    sprintf(macstring, "%02X:%02X:%02X:%02X:%02X:%02X", macaddress[0], macaddress[1], macaddress[2], macaddress[3],
...
void CBillingPlayer::sendIntervalValidation() {
    ...
    filelog(LOGFILE_CBILLING_PLAYER, "Request Interval Validation : %s\n%s\n%s", body.Parameter_Value,
            header.toString().c_str(), body.toString().c_str());
...
#elif defined(__GAME_SERVER__)
void CBillingPlayer::executeIntervalValidation(CBillingPacketHeader& header,
                                               CBillingPacketResponseIntervalValidationBody& body) {
    __BEGIN_TRY
    ...
    if (atoi(body.Parameter_Value) != g_pCBillingPlayerManager->getMinusIntervalInt()) {
        ...
        cerr << "gameserver interval : " << g_pCBillingPlayerManager->getMinusIntervalInt()
             << ", china billing server interval : " << body.Parameter_Value << endl;
        cerr << "-------------------------------------------------------------------------------" << endl;

        filelog(LOGFILE_CBILLING_ERROR,
                "Different Interval between gameserver and china billing server\ngameserver interval : %d, china "
    ...
    if (pCreature != NULL) {
        ...
        if (body.Free_Left_Time + body.Rating_Left_Time == 0) {
            ...
            filelog(LOGFILE_CBILLING_PACKET, "Response Login OK : %s has no left time. DISCONNECT\n%s\n%s",
                    body.Login_Name, header.toString().c_str(), body.toString().c_str());
        } else {
#ifdef __COUT_CBILLING_SYSTEM__
            cout << "----------------------------------------------------------------" << endl;
            cout << "Response Login OK : " << body.Login_Name << endl;
        ...
        //		char m[100];
        //		sprintf( m, "Free Left Point : %d.%d Left Point %d.%d", body.Free_Left_Time / 10, body.Free_Left_Time %
        // 10, body.Rating_Left_Time / 10, body.Rating_Left_Time % 10 ); 		GCSystemMessage msg;
        // msg.setMessage( m ); 		pGamePlayer->sendPacket( &msg );
    } else {
#ifdef __COUT_CBILLING_SYSTEM__
        cout << "----------------------------------------------------------------" << endl;
        cout << "Response Login OK : " << body.Login_Name << endl;
...
#ifdef __COUT_CBILLING_SYSTEM__
        cerr << "******************************************************************" << endl;
        cerr << "Session Fail : Recv ID = " << header.Session_ID << ", Respect ID = " << pGamePlayer->getRecvID()
             << endl;
        cerr << header.toString().c_str() << endl;
#endif
        filelog(LOGFILE_CBILLING_ERROR, "Session Fail : RecvID = %d, Respect ID = %d\n%s", header.Session_ID,
                pGamePlayer->getRecvID(), header.toString().c_str());
    }
...
void CBillingPlayer::logPacket(CBillingPacketHeader* header, CBillingPacketResponseLoginBody* login,
    ...
    BEGIN_DB {
        ...
        pStmt->executeQuery("INSERT INTO TB_BillingLog( DateTime, PlayerID, PacketType, ReturnCode, PlayerType, "
                            "PayType, FreePayPoint, FreeLeftTime, PayPoint, LeftPayTime, EndDateTime ) VALUES ( NOW(), "
                            "'%s', '%s', '%s', '%s', '%s', %d, %d, %d, %d, '%s' )",
                            playerID.c_str(), methodCode.c_str(), returnCode.c_str(), playerType.c_str(),
                            payType.c_str(), freePayPoint, freeLeftTime, payPoint, leftPayTime,
                            endDateTime.toDateTime().c_str());
        SAFE_DELETE(pStmt);
    }
    END_DB(pStmt)
