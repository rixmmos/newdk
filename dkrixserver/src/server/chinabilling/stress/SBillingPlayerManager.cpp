            ...
            for (int i = 0; i < m_SBillingPlayers; ++i) {
                ...
                if (m_pSBillingPlayer[i] == NULL) {
                    ...
                    try {
                        ...
                        cout << "connection to china billing server established - " << SBillingServerIP.c_str() << ":"
                             << SBillingServerPort << endl;
                        ...
                        cout << "connect to china billing server fail - " << SBillingServerIP.c_str() << ":"
                             << SBillingServerPort << endl;
                        filelog(LOGFILE_CBILLING_PLAYER, "connect failed(%s:%d)", SBillingServerIP.c_str(),
                                SBillingServerPort);
                    ...
                    if (m_pSBillingPlayer[i]->getSocket()->getSockError()) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                filelog(LOGFILE_CBILLING_PLAYER, "[SBillingPlayer release error]%s",
                                        t.toString().c_str());
                            }
                        }
                    }
