...
int LogData::read(int sockfd) {
    ...
    // type
    if (recv(sockfd, &m_Type, szshort, 0) == 0)
        return 0;

    // zone id
    if (recv(sockfd, &m_ZoneID, szshort, 0) == 0)
        return 0;

    // source
    if (recv(sockfd, &length, szBYTE, 0) == 0)
        return 0;
    if (length != 0) {
        memset(buf, 0, sizeof(char) * (LOGDATA_BUFFER_SIZE + 1));
        if (recv(sockfd, buf, length, 0) == 0)
            return 0;
        m_Source = string(buf);
    }
    ...
    // content
    if (recv(sockfd, &length2, szshort, 0) == 0)
        return 0;
    if (length2 != 0) {
        memset(buf, 0, sizeof(char) * (LOGDATA_BUFFER_SIZE + 1));
        if (recv(sockfd, buf, length2, 0) == 0)
            return 0;
...
void LogData::write(int sockfd) {
    ...
    if (m_Source.size() != 0)
        send(sockfd, m_Source.c_str(), m_Source.size(), 0);

    // target
    length = m_Target.size();
    send(sockfd, &length, szBYTE, 0);
    if (m_Target.size() != 0)
        send(sockfd, m_Target.c_str(), m_Target.size(), 0);

    // content
    length2 = m_Content.size();
    send(sockfd, &length2, szshort, 0);
    if (m_Content.size() != 0)
        send(sockfd, m_Content.c_str(), m_Content.size(), 0);
}
