        ...
        while (true) {
            ...
            if (m_pBillingPlayer == NULL) {
                ...
                try {
                    ...
                    cout << "connection to billingServer established -  " << BillingServerIP.c_str() << ":"
                         << BillingServerPort << endl;
                    filelog(LOGFILE_BILLING_PLAYER, "----- connection established(%s:%d) -----",
                            BillingServerIP.c_str(), BillingServerPort);
                } catch (Throwable& t) {
                    cout << "connect to billingServer fail - " << BillingServerIP.c_str() << ":" << BillingServerPort
                         << endl;
                    filelog(LOGFILE_BILLING_PLAYER, "connection failed(%s:%d)", BillingServerIP.c_str(),
                            BillingServerPort);
