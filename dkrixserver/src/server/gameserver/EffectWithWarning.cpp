...
void EffectIcicle::affect() {
    ...
    for (; dir <= 8; dir += 2) {
        ...
        for (; itr != endItr; ++itr) {
            ...
            if (pCreature == NULL || pCreature->isFlag(Effect::EFFECT_CLASS_NO_DAMAGE))
                continue;
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (isValidZoneCoord(m_pZone, X, Y)) {
            ...
            for (; itr != endItr; ++itr) {
                ...
                //				if ( finalHP != 0 )
                //					knockbackCreature( m_pZone, pCreature, X - dirMoveMask[(m_Dir+2)%8].x, Y -
                // dirMoveMask[(m_Dir+2)%8].y );
        ...
        for (; itr != endItr; ++itr) {
            ...
            knockbackCreature(m_pZone, pCreature, X - dirMoveMask[(m_Dir + 2) % 8].x,
                              Y - dirMoveMask[(m_Dir + 2) % 8].y);
        }
