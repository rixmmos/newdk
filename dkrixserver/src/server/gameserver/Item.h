...
public:
    ...
    virtual bool isSilverWeapon() const {
        return false;
    }
    ...
    void setItemID(ItemID_t itemID) {
        m_ItemID = itemID;
    }
    ...
    virtual Durability_t getMaxDurability() const {
        return 1;
    }
    ...
    virtual BYTE getBulletCount() const {
        return 0;
    }
    ...
    virtual Damage_t getBonusDamage() const {
        return 0;
    }
    ...
    virtual Grade_t getGrade() const {
        return -1;
    }
    virtual void setGrade(Grade_t Grade) {}

    virtual Luck_t getLuck() const {
        return 0;
    }
    ...
    bool isFlag(Effect::EffectClass Flag) {
    ...
    void setQuestItem(bool bQuest = true) {
        m_bQuest = bQuest;
    }

    bool isFlagItem() const {
        return getItemClass() == ITEM_CLASS_EVENT_ITEM && getItemType() == 27;
    }
    bool isSweeper() const {
        return getItemClass() == ITEM_CLASS_SWEEPER;
    }

    bool isTraceItem() const {
        return m_bTrace;
    }
    void setTraceItem(bool bTrace = false) {
        m_bTrace = bTrace;
    }

    bool isTimeLimitItem() const {
        return m_bTimeLimit;
    }
    void setTimeLimitItem(bool bTimeLimit = true) {
        m_bTimeLimit = bTimeLimit;
    }
    ...
    bool isOnStore() const {
        return m_bOnStore;
    }

    void displayItem(Store* pStore) {
        m_bOnStore = true;
        setStore(pStore);
    }
    void undisplayItem() {
