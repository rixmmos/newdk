...
public:
    virtual ItemClass getItemClass() const {
        return Item::ITEM_CLASS_TRAP_ITEM;
    }
    virtual string getObjectTableName() const {
        return "TrapItemObject";
    }
...
public:
    ...
    void setFunction(uint fun) {
        m_Function = fun;
    }

    uint getParameter() const {
        return m_Parameter;
    }
    void setParameter(uint param) {
        m_Parameter = param;
    }
...
public:
    virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) {
        return new TrapItem(ItemType, OptionType);
    }
};
