...
public:
    virtual ItemClass getItemClass() const {
        return Item::ITEM_CLASS_MAGAZINE;
    }
...
public:
    ...
    enum GunType { AR = 0, SMG, SG, SR };

public:
    uint getMaxBullets() const {
        return m_MaxBullets;
    }
    void setMaxBullets(uint maxBullets) {
        m_MaxBullets = maxBullets;
    }

    Silver_t getMaxSilver(void) const {
        return m_MaxSilver;
    }
    void setMaxSilver(Silver_t silver) {
        m_MaxSilver = silver;
    }
...
public:
    virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) {
        return new Magazine(ItemType, OptionType, 1);
    }
};
