...
public:
    virtual ItemClass getItemClass() const {
        return Item::ITEM_CLASS_EVENT_BALL;
    }
    virtual string getObjectTableName() const {
        return "EventBallObject";
    }
...
public:
    virtual Item* createItem(ItemType_t ItemType, OptionType_t OptionType) {
        return new EventBall(ItemType, OptionType, 1);
    }
};
