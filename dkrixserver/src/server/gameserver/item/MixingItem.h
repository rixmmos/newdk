...
public:
    virtual ItemClass getItemClass() const {
        return Item::ITEM_CLASS_MIXING_ITEM;
    }
    virtual string getObjectTableName() const {
        return "MixingItemObject";
    }
...
public:
    enum Target { MIXING_WEAPON, MIXING_ARMOR, MIXING_ACCESSORY, MIXING_ALL };

    enum Type { TYPE_MIX, TYPE_DETACH, TYPE_CLEAR };
...
public:
    ...
    ItemType_t getVampireLevel() const {
        return m_VampireLevel;
    }
    ItemType_t getSlayerLevel() const {
        return m_SlayerLevel;
    }
    ItemType_t getOustersLevel() const {
        return m_OustersLevel;
    }

    void setVampireLevel(ItemType_t level) {
        m_VampireLevel = level;
    }
    void setSlayerLevel(ItemType_t level) {
        m_SlayerLevel = level;
    }
    void setOustersLevel(ItemType_t level) {
        m_OustersLevel = level;
    }
...
public:
    virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) {
        return new MixingItem(ItemType, OptionType, 1);
    }
};
