...
public:
    //	virtual ItemClass getItemClass() const  { return Item::ITEM_CLASS_RING; }
    //	virtual string getObjectTableName() const  { return "RingObject"; }
...
public:
    virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) {
        return new Ring(ItemType, OptionType);
    }
};
