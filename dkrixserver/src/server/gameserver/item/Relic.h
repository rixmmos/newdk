...
public:
    void setRelicType(const string& str) {
        if (str == RelicType2String[RELIC_TYPE_SLAYER])
            relicType = RELIC_TYPE_SLAYER;
        else if (str == RelicType2String[RELIC_TYPE_VAMPIRE])
            relicType = RELIC_TYPE_VAMPIRE;
        else
            throw InvalidProtocolException("Invalid Relic Type value");
    }
...
public:
    virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) {
        return new Relic(ItemType, OptionType);
    }
};
