...
public:
    virtual ItemClass getItemClass() const {
        return Item::ITEM_CLASS_QUEST_ITEM;
    }
...
public:
    ...
    Ratio_t getBonusRatio() const {
        return m_BonusRatio;
    }
    void setBonusRatio(Ratio_t BonusRatio) {
        m_BonusRatio = BonusRatio;
    }
...
public:
    virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) {
        return new QuestItem(ItemType, OptionType);
    }
};
