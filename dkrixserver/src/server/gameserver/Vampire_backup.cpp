...
void increaseVampExpEx(Vampire* pVampire, Exp_t Point) {
    ...
    if (NewGoalExp == 0 && curLevel < 115) {
        ...
        /*
        StringStream sav;
        sav << "Level = " << (int)curLevel << "," << "Exp = " << (int)pBeforeExpInfo->getAccumExp() << "," << "GoalExp =
        " << (int)NextGoalExp << "," << "Bonus = " << (int)bonus; pVampire->tinysave(sav.toString());
        */

        char pField[80];
        sprintf(pField, "Level=%d, Exp=%ld, GoalExp=%ld, Bonus=%d", curLevel, pBeforeExpInfo->getAccumExp(),
                NextGoalExp, bonus);
        pVampire->tinysave(pField);
    }
}
...

{
    ...
    msg << "Vampire("
        ...
        << ",Name:" << m_Name << ",HairColor:" << (int)m_HairColor << ",SkinColor:" << (int)m_SkinColor
        ...
        << ",HP:" << (int)m_HP[ATTR_CURRENT] << "/" << (int)m_HP[ATTR_MAX] << ",Fame:" << (int)m_Fame
        << ",Exp:" << (int)m_Exp << ",ExpOffset:" << (int)m_ExpOffset << ",Level:" << (int)m_Level << ",Bonus:"
        << (int)m_Bonus
        //<< ",InMagics:'" << ??? << "'"
