    ...
    for (int i = 0; i < m_Unit; ++i) {
        ...
        for (int j = 0; j < 100; ++j) {
            ...
            if (isValidZoneCoord(m_pZone, x, y)) {
                if (m_pZone->getTile(x, y).canAddEffect() && m_pZone->getTile(x, y).getEffect(m_IcicleEffect) == NULL &&
                    m_pZone->getTile(x, y).getEffect((EffectClass)(m_IcicleEffect + 1)) == NULL &&
                    !m_pZone->getTile(x, y).isFixedGroundBlocked())
                    break;
            }
...
void EffectCastingSideTrap::affect() {
    ...
    for (int i = 0; i < m_Unit; ++i) {
        ...
        while (true) {
            ...
            if (isValidZoneCoord(m_pZone, x, y)) {
                if (m_pZone->getTile(x, y).canAddEffect() &&
                    ...
                    m_pZone->getTile(x, y).getEffect(
                        (Effect::EffectClass)(Effect::EFFECT_CLASS_ICE_STICK_SW + dir180 - 1)) == NULL &&
                    m_pZone->getTile(x, y).getEffect(
                        (Effect::EffectClass)(Effect::EFFECT_CLASS_ICE_STICK_SW + dir180)) == NULL &&
                    //					!m_pZone->getTile(x + dirMoveMask[dir180].x*8, y +
                    // dirMoveMask[dir180].y*8).hasEffect() &&
                    m_pZone->getTile(x + dirMoveMask[dir180].x * 8, y + dirMoveMask[dir180].y * 8)
                            .getEffect((Effect::EffectClass)(Effect::EFFECT_CLASS_ICE_STICK_SW + ((dir180 + 4) % 8) -
                                                             1)) == NULL &&
                    m_pZone->getTile(x + dirMoveMask[dir180].x * 8, y + dirMoveMask[dir180].y * 8)
                            .getEffect((Effect::EffectClass)(Effect::EFFECT_CLASS_ICE_STICK_SW + ((dir180 + 4) % 8))) ==
                        NULL)
        ...
        EffectSideTrap* pEffectSideTrap =
            new EffectSideTrap((Effect::EffectClass)(Effect::EFFECT_CLASS_ICE_STICK_SW + dir180 - 1), m_pZone, x, y);
    ...
    for (int i = 0; i < trapCount; ++i) {
        ...
        for (int j = 0; j < m_WallLength; ++j) {
            ...
            for (int k = -2; k <= 2; ++k) {
                ...
                if (isValidZoneCoord(m_pZone, X, Y) && m_pZone->getTile(X, Y).canAddEffect() &&
                    !m_pZone->getTile(X, Y).isFixedGroundBlocked()) {
