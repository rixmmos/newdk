...
public:
    MonsterKillQuestStatus(QuestID_t qID, const VSDateTime& deadline, SpriteType_t targetSType, bool isChief, int goal)
        : QuestStatus(qID, deadline, QUEST_CLASS_MONSTER_KILL) {
        m_TargetMonsterSpriteType = targetSType;
        m_IsChief = isChief;
        m_GoalNum = goal;
        m_CurrentNum = 0;
    }
