...
ItemRewardInfo::ItemRewardInfo(RewardID_t rID, RewardClass_t rClass, Item::ItemClass iClass, ItemType_t iType,
                               const string& option, DWORD time)
...

{
    ...
    if (pInventory->getEmptySlot(pItemInfo->getVolumeWidth(), pItemInfo->getVolumeHeight(), tp))
...

{
    ...
    if (!pInventory->addItem(pItem, tp)) {
        SAFE_DELETE(pItem);
        filelog("Quest.log", "ItemRewardInfo::giveReward() : canGiveReward()     .");
        return COMPLETE_FAIL_NO_INVENTORY_SPACE;
    }
