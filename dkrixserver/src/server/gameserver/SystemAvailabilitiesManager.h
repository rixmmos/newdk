...
public:
    ...
    bool isAvailable(SystemKind kind) const {
        return m_SystemFlags.test(kind);
    }
    ...
    static void AssertAvailable(SystemKind kind, const string& msg) {
        if (getInstance()->isAvailable(kind))
            return;
        filelog("SystemAvailabilities.log", msg.c_str());
        throw DisconnectException(msg);
    }

    int getZoneOpenDegree() const {
        return m_ZoneOpenDegree;
    }
