...
void SiegeManager::start(ZoneID_t zoneID) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            if (j != i)
                pEffect->getSharingCreatures().push_back(pGate[j]->getObjectID());
        }
    ...
    static TPOINT guardPosition[] = {{47, 193},  {48, 195},  {48, 192},  {49, 194},  {63, 208}, {64, 207},
                                     {64, 210},  {65, 209},  {150, 89},  {151, 88},  {151, 91}, {152, 90},
...
void SiegeManager::recallGuild(ZoneID_t currentZoneID, ZoneID_t siegeZoneID, GuildID_t guildID, int side, int num) {
    ...
    for (list<Creature*>::iterator itr = clist.begin(); itr != clist.end(); ++itr) {
        ...
        if (pTargetCreature->getZone() == NULL)
            continue;
