...
public:
    ...
    public:
        ActionWalk(Monster* pTarget, ZoneCoord_t X, ZoneCoord_t Y, Turn_t delay)
            : Action(pTarget), m_TargetX(X), m_TargetY(Y), m_MoveDelay(delay), m_DodgeCount(0) {}
        ActionType getActionType() const {
            return ACTION_WALK;
        }
        Turn_t execute();
    ...
    public:
        ActionEffect(Monster* pTarget, Effect::EffectClass eClass, Turn_t duration)
            : Action(pTarget), m_EffectClass(eClass), m_Duration(duration) {}
        ActionType getActionType() const {
...
public:
    ...
    public:
        list<SummonInfo*>& getSummonInfos() {
            return m_SummonInfos;
        }
        const list<SummonInfo*>& getSummonInfos() const {
            return m_SummonInfos;
        }
    ...
    MonsterSummonState(Zone* pZone, DWORD clearState, DWORD failState, DWORD timeoutState, Turn_t timeLimit)
        : TimerState(timeoutState, timeLimit), m_pZone(pZone), m_ClearState(clearState), m_FailState(failState),
          m_TimeLimit(true) {}
    MonsterSummonState(Zone* pZone, DWORD clearState, DWORD failState)
        : TimerState(clearState, 999999999), m_pZone(pZone), m_ClearState(clearState), m_FailState(failState),
          m_TimeLimit(false) {}
