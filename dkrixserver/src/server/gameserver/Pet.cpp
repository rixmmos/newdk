...
bool AttackingPet::useSkill() {
    ...
    if (!verifyDistance(pOwner, pTarget, range))
        return false;
    ...
    pZone->broadcastSkillPacket(pOwner->getX(), pOwner->getY(), pTarget->getX(), pTarget->getY(), &gcPetUseSkill,
                                list<Creature*>());
...
bool Pixie::useSkill() {
    ...
    if (pOwner->getMP(ATTR_MAX) <= pOwner->getMP(ATTR_CURRENT))
        return false;
    ...
    if (pTarget != NULL) {
        if (pTarget->isFlag(Effect::EFFECT_CLASS_CANNOT_ABSORB_SOUL) || !pTarget->isFlag(Effect::EFFECT_CLASS_COMA))
            return false;
        X = pTarget->getX();
        Y = pTarget->getY();
    } else if (pTargetCorpse != NULL) {
        if (pTargetCorpse->isFlag(Effect::EFFECT_CLASS_CANNOT_ABSORB_SOUL))
            return false;
