...
class Trigger {
public:
    enum TriggerType { NPC_TRIGGER, QUEST_TRIGGER, MONSTER_TRIGGER };

    enum TriggerMode { ACTIVE_TRIGGER, PASSIVE_TRIGGER, NEUTRAL_TRIGGER };
...
public:
    ...
    bool isAllSatisfied(TriggerMode triggerMode, Creature* pCreature1, Creature* pCreature2 = NULL,
...
public:
    ...
    void setTriggerType(const string& triggerType) {
        ...
        else if (triggerType == "QUEST")
            m_TriggerType = QUEST_TRIGGER;
        else if (triggerType == "MONSTER")
            m_TriggerType = MONSTER_TRIGGER;
        else
            throw Error("invalid triggerType");
    }

    TriggerID_t getTriggerID() const {
        return m_TriggerID;
    }
    void setTriggerID(TriggerID_t triggerID) {
    ...
    const ConditionSet& getConditionSet() const {
        return m_ConditionSet;
    }
