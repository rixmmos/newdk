...
//--------------------------------------------------------------------------------
// Phase 9C (2026-04-19): Range-checked Lua-stack → enum cast.
//
// Quest scripts supply enum values as plain Lua numbers. The old
// pattern was:
//
...
//   - out-of-range integers that reinterpret-cast to a valid-
//     looking enumerator and crash downstream code holding an
//     enum-indexed dispatch table
//   - negative numbers that wrap when the enum is typedef'd to
//     an unsigned underlying type
//
// lua_toboundedenum<T>() rejects all three with an
// InvalidProtocolException carrying the slot index, actual value,
// and the legal range so misbehaving quest scripts fail loudly
// with a breadcrumb.
//
// Usage:
...
inline T lua_toboundedenum(lua_State* L, int idx, long minValue, long maxValue) {
    if (!lua_isnumber(L, idx)) {
        ...
        msg << "lua_toboundedenum: stack slot " << idx
            << " is not a number (Lua type=" << lua_type(L, idx) << ")";
        throw InvalidProtocolException(msg.str());
    }
    long v = (long)lua_tonumber(L, idx);
    if (v < minValue || v > maxValue) {
        std::ostringstream msg;
        msg << "lua_toboundedenum: value " << v
            << " on stack slot " << idx
            << " is outside [" << minValue << ", " << maxValue << "]";
        throw InvalidProtocolException(msg.str());
    }
    return (T)v;
}
