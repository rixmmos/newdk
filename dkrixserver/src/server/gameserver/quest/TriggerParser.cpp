...
void TriggerParser::parseTrigger(const string& type, const string& condition, const string& action) {
    ...
    if (m_pTargetTree == NULL)
        m_pTargetTree = new XMLTree("Triggers");
    if (m_TargetScriptID == 0) {
        ...
        for (size_t i = 0; i < pScriptTree->GetChildCount(); ++i) {
            ...
            if (pScriptChild != NULL && pScriptChild->GetName() == "Content" &&
                static_cast<uint>(pScriptChild->GetAttribute("AnswerID")->ToInt()) == m_TargetContentID) {
