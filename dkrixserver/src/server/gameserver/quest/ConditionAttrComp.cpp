...

{
    ...
    if (m_ReqSTR > 0) {
        if (pCreature2->isSlayer()) {
            ...
            if (!satisfy(m_STROperator, current, m_ReqSTR))
                return false;
    ...
    if (m_ReqDEX > 0) {
        if (pCreature2->isSlayer()) {
            ...
            if (!satisfy(m_DEXOperator, current, m_ReqDEX))
                return false;
        } else if (pCreature2->isVampire()) {
            Vampire* pVampire = dynamic_cast<Vampire*>(pCreature2);
    ...
    if (m_ReqINT > 0) {
        if (pCreature2->isSlayer()) {
            ...
            if (!satisfy(m_INTOperator, current, m_ReqINT))
                return false;
        } else if (pCreature2->isVampire()) {
            Vampire* pVampire = dynamic_cast<Vampire*>(pCreature2);
    ...
    if (m_ReqSum > 0) {
        if (pCreature2->isSlayer()) {
            ...
            if (!satisfy(m_SumOperator, current, m_ReqSum))
                return false;
        } else if (pCreature2->isVampire()) {
            Vampire* pVampire = dynamic_cast<Vampire*>(pCreature2);
    ...
    if (m_ReqLevel > 0) {
        if (pCreature2->isSlayer()) {
            ...
            if (!satisfy(m_LevelOperator, current, m_ReqLevel))
                return false;
        } else if (pCreature2->isOusters()) {
            Ousters* pOusters = dynamic_cast<Ousters*>(pCreature2);
...

{
    ...
    if (op == "LOWER")
        var = CONDITION_ATTR_COMP_LOWER;
    else if (op == "EQUAL")
        var = CONDITION_ATTR_COMP_EQUAL;
    else if (op == "HIGHER")
