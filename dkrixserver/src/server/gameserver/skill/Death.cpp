...

{
    ...
    try {
        ...
        if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bHitRoll2 && bCanHit && !bEffected && bPK) {
            ...
            if ((resist * 10 / 3) > output.Duration)
                output.Duration = 0;
            else
                output.Duration -= resist * 10 / 3;
...

{
    ...
    bool bEffected = pEnemy->isFlag(Effect::EFFECT_CLASS_DEATH) || pEnemy->isFlag(Effect::EFFECT_CLASS_IMMUNE_TO_CURSE);
