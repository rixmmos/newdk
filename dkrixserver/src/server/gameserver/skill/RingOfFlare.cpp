...

{
    ...
    try {
        ...
        if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && bSatisfyRequire) {
            ...
            if (Grade > 0) {
                if (Grade == 1)
                    pEffect->setSendEffectClass(Effect::EFFECT_CLASS_RING_OF_FLARE_2);
                else
                    pEffect->setSendEffectClass(Effect::EFFECT_CLASS_RING_OF_FLARE_3);
            } else
                pEffect->setSendEffectClass(Effect::EFFECT_CLASS_RING_OF_FLARE);
