...

{
    ...
    try {
        ...
        bool bHitRoll =
            HitRoll::isSuccessVampireCurse(pSkillInfo->getLevel(), pTargetCreature->getResist(MAGIC_DOMAIN_CURSE));
        ...
        bool bEffected = pTargetCreature->isFlag(Effect::EFFECT_CLASS_DOOM);
        ...
        if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bHitRoll2 && bCanHit && !bEffected && bPK) {
            ...
            pEffect->setProtectionPenalty(output.Damage);
            ...
            gcAddEffect.setEffectID(Effect::EFFECT_CLASS_DOOM);
...

{
    ...
    SkillType_t SkillType = SKILL_DOOM;
    SkillInfo* pSkillInfo = g_pSkillInfoManager->getSkillInfo(SkillType);
    ...
    bool bEffected = pEnemy->isFlag(Effect::EFFECT_CLASS_DOOM) || pEnemy->isFlag(Effect::EFFECT_CLASS_IMMUNE_TO_CURSE);
