...

{
    ...
    BYTE Grade = 0;
    if (pOustersSkillSlot->getExpLevel() <= 10)
        Grade = 0;
    else if (pOustersSkillSlot->getExpLevel() <= 20)
        Grade = 1;
            ...
            for (int oX = pt.x - 2; oX <= pt.x + 2; ++oX)
                for (int oY = pt.y - 2; oY <= pt.y + 2; ++oY) {
                    ...
                    if (pZone->getTile(oX, oY).getEffect(Effect::EFFECT_CLASS_DARKNESS) != NULL ||
                        ...
                        pZone->getTile(oX, oY).getEffect(Effect::EFFECT_CLASS_HEAVEN_GROUND) != NULL ||
                        pZone->getTile(oX, oY).getEffect(Effect::EFFECT_CLASS_GRAY_DARKNESS) != NULL) {
                        bTileCheck = false;
                        break;
                    }
                }
            ...
            for (oX = X - 2; oX <= X + 2; ++oX)
                for (oY = Y - 2; oY <= Y + 2; ++oY) {
                    ...
                    if (pOldEffect != NULL) {
                        ...
                        ObjectID_t effectID = pOldEffect->getObjectID();
                        pZone->deleteEffect(effectID);
                        // pOldEffect->setDeadline(0);
                        // pOldEffect->setNextTime(0);
                    }
                    //////////////////////////////////////////////////////////////////////////
