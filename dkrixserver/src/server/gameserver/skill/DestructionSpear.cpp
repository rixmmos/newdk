...
void DestructionSpear::execute(Ousters* pOusters, ZoneCoord_t X, ZoneCoord_t Y, OustersSkillSlot* pOustersSkillSlot,
                               CEffectID_t CEffectID)
    ...
    for (; itr != result.targetCreatures.end(); ++itr) {
        ...
        if (!pTargetCreature->isFlag(Effect::EFFECT_CLASS_DESTRUCTION_SPEAR) && result.bSuccess) {
            ...
            if (input.SkillLevel <= 15) {
                ...
                ratio =
                    max(20, min(80, (int)(pOusters->getLevel() + 20.0 + (input.SkillLevel * 4.0 / 3.0) - targetLevel)));
            }
