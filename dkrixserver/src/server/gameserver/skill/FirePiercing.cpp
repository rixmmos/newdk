        ...
        if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bSatisfyRequire) {
            ...
            for (oX = (int)X - 1; oX <= (int)X + 1; ++oX)
                for (oY = (int)Y - 1; oY <= (int)Y + 1; ++oY) {
                    if (!isValidZoneCoord(pZone, oX, oY))
                        continue;
                        ...
                        if (pObject->getObjectClass() == Object::OBJECT_CLASS_CREATURE) {
                            ...
                            if (pCreature->getMoveMode() != Creature::MOVE_MODE_FLYING) {
                                if (pCreature->isSlayer()) {
                                    ...
                                    ::setDamage(pSlayer, Damage, pOusters, SKILL_FIRE_PIERCING, &_GCSkillToTileOK2,
                                                &_GCSkillToTileOK1);
                                    ...
                                    ::setDamage(pVampire, Damage, pOusters, SKILL_FIRE_PIERCING, &_GCSkillToTileOK2,
                                                &_GCSkillToTileOK1);
