...
public:
    ...
    PacketSize_t getPacketSize() const {
        return szSkillType + szCoord * 2 + szRange + szDuration + szBYTE + szObjectID * m_CListNum + szBYTE;
    }
    // CListNum, SListNum, ListEle* CListNum, ListEle* SListNum* 4

    // get packet's name
    string getPacketName() const {
        return "GCSkillToTileOK4";
    }
...
public:
    ...
    // get packet id
    PacketID_t getPacketID() const {
        return Packet::PACKET_GC_SKILL_TO_TILE_OK_4;
    }

    // get Pakcet Max Size
    PacketSize_t getPacketMaxSize() const {
        return szSkillType + szCoord * 2 + szRange + szDuration + szBYTE + szWORD + szObjectID + 255 + szBYTE;
    }
};
