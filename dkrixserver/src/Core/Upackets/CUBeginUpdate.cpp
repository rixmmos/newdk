...
void CUBeginUpdate::write(SocketOutputStream& oStream) const throw(ProtocolException, Error) {
    ...
    if (m_Version >= atoi(g_pConfig->getProperty("GuildVersion").c_str()))
        oStream.write(m_GuildVersion);
    if (m_Version >= atoi(g_pConfig->getProperty("InfoVersion").c_str()))
        oStream.write(m_InfoVersion);
