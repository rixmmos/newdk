...
int FileAPI::open_ex(const char* filename, int flags) {
    ...
    if (fd < 0) {
        ...
        case ENOTDIR:
            throw Error("A component used as a directory in pathname is not, in fact, a  directory, or O_DIRECTORY was "
                        "specified and pathname was not a directory.");
        case ENXIO:
            throw Error("O_NONBLOCK | O_WRONLY is set, the named file id a FIFO and no process has the file open for "
        ...
        case ETXTBSY:
            throw Error("pathname refers to an executable image which is currently being executed and write access was "
                        "requested.");
        case EFAULT:
            throw Error("pathname points outside your accessible address space.");
        case ELOOP:
            throw Error("Too many symbolic links were encountered in resolving pathname, or O_NOFOLLOW was specified "
...
int FileAPI::open_ex(const char* filename, int flags, int mode) {
    ...
    if (fd < 0) {
        ...
        case EISDIR:
            throw Error("pathname refers to a directory and the access requested involved writing.");
        case EACCES:
            throw FileNotOpenedException("The requested access to the file is not allowed, or one of the directories "
...
uint FileAPI::read_ex(int fd, void* buf, uint len) {
    ...
    if (result < 0) {
        ...
        case EAGAIN:
            throw NonBlockingIOException("Non-blocking I/O has been selected using O_NONBLOCK and no data was "
...
uint FileAPI::write_ex(int fd, const void* buf, uint len) {
    ...
    if (result < 0) {
        ...
        switch (errno) {
        case EAGAIN:
            throw NonBlockingIOException("Non-blocking I/O has been selected using O_NONBLOCK and there was no room in "
                                         "the pipe or socket connected to fd to write the data immediately.");
        case EINTR:
...
void FileAPI::ioctl_ex(int fd, int request, void* argp) {
    ...
    if (ioctl(fd, request, argp) < 0) {
        ...
        case ENOTTY:
            throw Error("fd is not associated with a character special device. or The specified request does not apply "
                        "to the kind of object that the descriptor fd references.");
        case EINVAL:
