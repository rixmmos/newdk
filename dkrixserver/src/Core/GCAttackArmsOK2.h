...
public:
    ...
    PacketID_t getPacketID() const {
        return PACKET_GC_ATTACK_ARMS_OK_2;
    }

    // get packet's body size
    
    PacketSize_t getPacketSize() const {
        return szSkillType + szObjectID + ModifyInfo::getPacketSize();
    }

    // get packet's name
    string getPacketName() const {
        return "GCAttackArmsOK2";
    }
...
public:
    ...
    // get packet id
    PacketID_t getPacketID() const {
        return Packet::PACKET_GC_ATTACK_ARMS_OK_2;
    }

    // get Packet Max Size
    PacketSize_t getPacketMaxSize() const {
        return szSkillType + szObjectID + ModifyInfo::getPacketMaxSize();
    }
};
