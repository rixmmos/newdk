...
public:
    ...
    PacketID_t getPacketID() const {
        return PACKET_GC_SKILL_TO_OBJECT_OK_6;
    }

    // get packet's body size
    
    PacketSize_t getPacketSize() const {
        return szCoord * 2 + szSkillType + szDuration + szBYTE + ModifyInfo::getPacketSize();
    }

    // get packet's name
    string getPacketName() const {
        return "GCSkillToObjectOK6";
    }
...
public:
    ...
    // get packet id
    PacketID_t getPacketID() const {
        return Packet::PACKET_GC_SKILL_TO_OBJECT_OK_6;
    }

    // get Packet Max Size
    PacketSize_t getPacketMaxSize() const {
        return szCoord * 2 + szSkillType + szDuration + szBYTE + ModifyInfo::getPacketMaxSize();
    }
};
