        ...
        if (pTargetCreature != NULL && pTargetCreature->isPC()) {
            ...
            if ((pCreature->isSlayer() || pCreature->getCompetenceShape() != 1) && pTargetCreature->isSlayer()) {
                ...
                if (pSlayer->getSkillDomainLevel(SKILL_DOMAIN_SWORD) == 150)
                    gcOtherModifyInfo.addShortData(MODIFY_SWORD_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_SWORD) + advLVL);
                else
                    gcOtherModifyInfo.addShortData(MODIFY_SWORD_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_SWORD));
                if (pSlayer->getSkillDomainLevel(SKILL_DOMAIN_BLADE) == 150)
                    gcOtherModifyInfo.addShortData(MODIFY_BLADE_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_BLADE) + advLVL);
                else
                    gcOtherModifyInfo.addShortData(MODIFY_BLADE_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_BLADE));
                if (pSlayer->getSkillDomainLevel(SKILL_DOMAIN_HEAL) == 150)
                    gcOtherModifyInfo.addShortData(MODIFY_HEAL_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_HEAL) + advLVL);
                else
                    gcOtherModifyInfo.addShortData(MODIFY_HEAL_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_HEAL));
                if (pSlayer->getSkillDomainLevel(SKILL_DOMAIN_ENCHANT) == 150)
                    gcOtherModifyInfo.addShortData(MODIFY_ENCHANT_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_ENCHANT) + advLVL);
                else
                    gcOtherModifyInfo.addShortData(MODIFY_ENCHANT_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_ENCHANT));
                if (pSlayer->getSkillDomainLevel(SKILL_DOMAIN_GUN) == 150)
                    gcOtherModifyInfo.addShortData(MODIFY_GUN_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_GUN) + advLVL);
                else
                    gcOtherModifyInfo.addShortData(MODIFY_GUN_DOMAIN_LEVEL,
                                                   pSlayer->getSkillDomainLevel(SKILL_DOMAIN_GUN));
                ...
                bSendInfo = true;
            } else if ((pCreature->isVampire() || pCreature->getCompetenceShape() != 1) &&
                ...
                bSendInfo = true;
            } else if ((pCreature->isOusters() || pCreature->getCompetenceShape() != 1) &&
