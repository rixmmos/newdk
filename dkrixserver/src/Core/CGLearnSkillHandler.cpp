...
// #endif
    ...
    if (pGamePlayer->getPlayerStatus() == GPS_NORMAL) {
        ...
        if (pPC->isSlayer())
            ...
            executeVampireSkill(pPacket, pPlayer);
        else if (pPC->isOusters())
            executeOustersSkill(pPacket, pPlayer);
        else
            throw ProtocolException("CGLearnSkillHandler::execute() : Unknown player creature!");
    } else
        throw ProtocolException("CGLearnSkillHandler::execute() : Game Player Status is not NORMAL!");
...

{
    ...
    if (pSkillInfo->getType() != SKILL_SOUL_CHAIN &&
        (pSkillInfo->getDomainType() != actualDomainType || actualDomainType > SKILL_DOMAIN_ETC)) {
        GCLearnSkillFailed failpkt;
        failpkt.setSkillType(targetSkillType);
        ;
        failpkt.setDesc(DOMAIN_DIFFER);
        pPlayer->sendPacket(&failpkt);
        filelog("learnSkillError.txt", "slayer domain error,player is %s,skill is :%s", pSlayer->getName().c_str(),
                pSkillInfo->getName().c_str());
        return;
    }
...

{
    ...
    // check skill type ,modified by chengh 2006 06 14
    if (pSkillInfo->getType() != SKILL_SOUL_CHAIN &&
        (pSkillInfo->getDomainType() != targetDomainType || targetDomainType != SKILL_DOMAIN_VAMPIRE)) {
        GCLearnSkillFailed failpkt;
        failpkt.setSkillType(targetSkillType);
        ;
        failpkt.setDesc(DOMAIN_DIFFER);
        pPlayer->sendPacket(&failpkt);
        filelog("learnSkillError.txt", "vampire domain error,player is %s,skill is:%s", pVampire->getName().c_str(),
                pSkillInfo->getName().c_str());
        return;
    }
...

{
    ...
    // check skill type ,modified by chengh 2006 06 14
    if (pSkillInfo->getType() != SKILL_SOUL_CHAIN &&
        (pSkillInfo->getDomainType() != targetDomainType || targetDomainType != SKILL_DOMAIN_OUSTERS)) {
        GCLearnSkillFailed failpkt;
        failpkt.setSkillType(targetSkillType);
        ;
        failpkt.setDesc(DOMAIN_DIFFER);
        pPlayer->sendPacket(&failpkt);
        filelog("learnSkillError.txt", "ousters domain error,player is: %s,skill is: %s", pOusters->getName().c_str(),
                pSkillInfo->getName().c_str());
        return;
    }
