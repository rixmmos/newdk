...
public:
    ...
    // get orign x, y
    Coord_t getOrgX() const {
        return m_OrgX;
    }
    Coord_t getOrgY() const {
        return m_OrgY;
    }
    // set origin x, y
    void setOrgXY(Coord_t X, Coord_t Y) {
        m_OrgX = X;
        m_OrgY = Y;
    }
...
public:
    ...
    // get packet id
    PacketID_t getPacketID() const {
        return Packet::PACKET_GC_SKILL_TO_TILE_OK_6;
    }

    // get Pakcet Max Size
    PacketSize_t getPacketMaxSize() const {
        return szCoord * 2 + szSkillType + szCoord * 2 + szRange + szDuration + szBYTE + szWORD + szObjectID + szBYTE +
