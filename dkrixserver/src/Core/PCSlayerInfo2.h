...
public:
    ...
    void setMasterEffectColor(BYTE masterEffectColor) {
        m_MasterEffectColor = masterEffectColor;
    }
    ...
    Attr_t getSTR(AttrType attrType = ATTR_CURRENT) const {
        if (m_STR[attrType] > maxSlayerAttr)
    ...
    Attr_t getDEX(AttrType attrType = ATTR_CURRENT) const {
        if (m_DEX[attrType] > maxSlayerAttr)
    ...
    Attr_t getINT(AttrType attrType = ATTR_CURRENT) const {
        if (m_INT[attrType] > maxSlayerAttr)
    ...
    SkillExp_t getSkillDomainExp(SkillDomain domain) const {
        return m_DomainExps[domain];
    }
    void setSkillDomainExp(SkillDomain domain, SkillExp_t skillExp) {
        m_DomainExps[domain] = skillExp;
    }

    void setSkillDomain(SkillDomain domain, SkillLevel_t skillLevel, SkillExp_t skillExp) {
        m_DomainLevels[domain] = skillLevel;
        m_DomainExps[domain] = skillExp;
    }
    ...
    Bonus_t getAttrBonus() const {
        return m_AttrBonus;
    }
    void setAttrBonus(Bonus_t bonus) {
        m_AttrBonus = bonus;
    }
