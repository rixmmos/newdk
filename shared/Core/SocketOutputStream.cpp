...
void SocketOutputStream::writePacket(const Packet* pPacket) {
    ...
    std::cout << "Send:" << packetID << "[" << packetSize << "," << (m_Sequence - 1) << "]" << " "
              << pPacket->toString() << std::endl;
#endif
    Assert(packetID != 0);
    ...
    if ((l2 - l1) != packetSize) {
        std::cout << "writePacket WARN: diff size = " << (l2 - l1) << " real size = " << packetSize << "before:" << l1
                  << " after:" << l2 << std::endl;
    }
    ...
    if( packetID == Packet::PACKET_GC_UPDATE_INFO ) {
        ...
        std::ofstream file("flush.txt", std::ios::out | std::ios::app);
        file << "SEND GCUPDATE INFO" << std::endl;
        file.close();
...
uint SocketOutputStream::flush() {
    ...
    try {
        ...
        std::cerr << "SocketOutputStream NonBlockingIOException Check! " << std::endl;
        ...
        std::cerr << "SocketOutputStream Exception Check! " << std::endl;
    ...
    /*
    std::ofstream file("flush.txt", std::ios::out | std::ios::app);
    file << "flush send size: " << nFlushed << " left size : " << nLeft << std::endl;
    file.close();
    */
...
void SocketOutputStream::resize(int size) {
    ...
    if (m_Socket == NULL) {
        ...
        ofile << m_Socket->getHost().c_str() << " - SocketOutputStream resized: " << orgSize << " / " << size << "/"
              << m_BufferLen << " bytes!"
              << std::endl;
        ofile.close();
    }
