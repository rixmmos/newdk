...
public:
    ...
    // Kept from the client's pre-migration copy (the server file had no
    // setter): the client fills in the MAC address before sending
    // (LCReconnectHandler.cpp). Wire-neutral; unused by the server.
    void setMacAddress(const BYTE* macAddress) {
        memcpy(m_MacAddress, macAddress, 6 * sizeof(BYTE));
    }
...
// Server-only: CGConnectHandler::execute has no client-side definition
