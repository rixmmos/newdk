...
void CGSMSSend::read(SocketInputStream& iStream) throw(ProtocolException, Error) {
    __BEGIN_TRY

    BYTE size;

    iStream.read(size);
    if (size > MAX_RECEVIER_NUM) size = MAX_RECEVIER_NUM;
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (strSize > MAX_NUMBER_LENGTH) strSize = MAX_NUMBER_LENGTH;
        iStream.read(number, strSize);
        m_Numbers.push_back(number);
    }

    iStream.read(size);
    if (size > MAX_NUMBER_LENGTH) size = MAX_NUMBER_LENGTH;
...
void CGSMSSend::write(SocketOutputStream& oStream) const throw(ProtocolException, Error) {
    __BEGIN_TRY
...
void CGSMSSend::execute(Player* pPlayer) throw(ProtocolException, Error) {
    __BEGIN_TRY
...
string CGSMSSend::toString() const throw() {
    __BEGIN_TRY
