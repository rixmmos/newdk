...
public:
    CGShopRequestSell() throw() {
        m_ObjectID = 0;
        m_ItemObjectID = 0;
        m_OpCode = SHOP_REQUEST_SELL_NORMAL;
    }
    virtual ~CGShopRequestSell() throw() {}
...
public:
    ...
    ObjectID_t getItemObjectID() throw() {
        return m_ItemObjectID;
    }
    void setItemObjectID(ObjectID_t oid) throw() {
        m_ItemObjectID = oid;
    }

    BYTE getOpCode(void) const throw() {
        return m_OpCode;
    }
    void setOpCode(BYTE code) throw() {
        m_OpCode = code;
    }
